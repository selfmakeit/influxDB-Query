...
//	private JsonObject anaylsisResult;
	...
	public int InputData(char[] inputBuffer, int inputLength) {
		...
		for (int i = 0; i < inputLength; i++) {
			dataStringBuilder.append(inputBuffer[i]);
	...
	public QueryResult Query( String sql) {
		...
		if (SendGet(getPath.toString())) {
			...
			if(0==resultCode){
				QueryResult queryResult=new QueryResult(this.dataStringBuilder.toString());
				return  queryResult;
			}
		}
...
//			JsonDeserializer jsonDeserializer = new JsonDeserializer();
			...
			while (receivedDataLenght < contentLength) {
				...
				if (jsonParseSuccess) {
					...
					int result = InputData(httpBodyReceiveBuffer, httpBodyReceiveBufferLength);
					if (result != 0) {
...
//		anaylsisResult = null;
//		JsonDeserializer jsonDeserializer = new JsonDeserializer();
		int chunkSize = ReceiveChunkedPart(bufferedReader);
		while (chunkSize > 0) {
			chunkSize = ReceiveChunkedPart(bufferedReader);
		}
		if (jsonParseSuccess) {
			return 0;
		} else {
	...
	private int ReceiveChunkedPart(BufferedReader bufferedReader) {
		try {
